...
namespace Cydb.Repository.Repository.EntZdgzFactory.FieldType {
    public class EntZdgzFieldTypeLrze : EntZdgzFieldBase {
        public EntZdgzFieldTypeLrze(EntZdgzRepository.EntZczbChangeDto entRankingDto) : base(entRankingDto) {
        }
        ...
        public override SqlBuildSubQuery BuildEntZdgzDto() {
            ...
            SqlBuildSubQuery.SubQuery = $@" ( select NAT_ORG_CODE, sum(LRZE) targetfield from T_CW_MONTH where rep_date={EntZczbDto.BeginTime} and FLAG_ORGINFO2=1 group by NAT_ORG_CODE ) a,
( select NAT_ORG_CODE, sum(LRZE) targetfield from T_CW_MONTH where rep_date={EntZczbDto.TbBeginTime} and FLAG_ORGINFO2=1 group by NAT_ORG_CODE ) b ";
