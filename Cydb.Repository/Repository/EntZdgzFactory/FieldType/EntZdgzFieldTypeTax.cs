...
namespace Cydb.Repository.Repository.EntZdgzFactory.FieldType {
    public class EntZdgzFieldTypeTax : EntZdgzFieldBase {
        ...
        public override SqlBuildSubQuery BuildEntZdgzDto() {
            ...
            SqlBuildSubQuery.SubQuery = $@" ( select nat_org_code, round(sum(tax)/10000,2) targetfield from t_tax3 where rep_date between {EntZczbDto.BeginTime} and {EntZczbDto.EndTime} group by nat_org_code ) a,
( select nat_org_code, round(sum(tax)/10000,2) targetfield from t_tax3 where rep_date between {EntZczbDto.TbBeginTime} and {EntZczbDto.TbEndTime} group by nat_org_code ) b ";
