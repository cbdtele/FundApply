...
namespace Cydb.Repository.Repository.EntRankFactory.FieldType {
    ...
    /// 企业排名 - 资产总计
    /// </summary>
    public class EntRankFieldTypeZczj : EntRankFieldTypeBase {
        ...
        public override SqlBuildSubQuery BuildEntRankingDto() {
            SqlBuildSubQuery.SubQuery = $@" ( select NAT_ORG_CODE, sum(ZCZJ) targetfield from T_CW_MONTH where rep_date={EntRankingDto.BeginTime} and FLAG_ORGINFO2=1 group by NAT_ORG_CODE ) a,
( select NAT_ORG_CODE, sum(ZCZJ) targetfield from T_CW_MONTH where rep_date={EntRankingDto.TbBeginTime} and FLAG_ORGINFO2=1 group by NAT_ORG_CODE ) b ";
