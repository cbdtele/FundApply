var detailview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderRow: function(target, fields, frozen, rowIndex, rowData){
	    ...
	    cc.push('<span class="datagrid-row-expander datagrid-row-expand" style="display:none;width:16px;height:30px;cursor:pointer;" />' + '<span style=" padding:0px; margin:0px;display:block;width:16px;height:30px;cursor:pointer;font-size:15px;">' + (rowIndex + 1) + '</span>');
					//盒子模型调整，独立分割占位
					//cc.push('<span style="display:block;width:16px;height:16px;cursor:pointer;" />');
					//cc.push('</div>');
	...
	onFindTarget: function (tt){
		var parentClass = tt.parent().parent().attr("id");
		var targetClass;
		if(parentClass){
			var strSplit = parentClass.split('-');
			var outStrSplit ="";
			for(var i = 0; i<strSplit.length;i++){
				if(i==3){
					outStrSplit += "1-";
				}else{
					outStrSplit += strSplit[i]+"-";
				}
			}
			targetClass=outStrSplit.substr(0,outStrSplit.length-1);
		}
		return targetClass;
	},
	...
	bindEvents: function(target){
		...
		body.unbind('click').bind('click', function(e){
			...
			if (tt.hasClass('datagrid-row-expand') || $("#"+detailview.onFindTarget(tt)+" span").hasClass('datagrid-row-expand')){
