...
(function($) {
	...
	var LG = 'linear-gradient(top, #fafafa, #eee)', CSS = '<style type="text/css">' +
		...
		'.wrap_remind{width:22em;padding:10px 10px;}' +
		'.wrap_remind p{margin:10px 0 0;}' +
        '.btnRemind{text-align:center;}' +
	...
	$.extend($.zxxbox, {
		...
		remind: function(message, callback, options) {
			var element = $('<div class="wrap_remind">'+message+'<p class="btnRemind"><button id="zxxSubmitBtn" class="submit_btn">确认</button</p></div>');
			$.zxxbox(element, options);
...
var showAlert = function (msg) {
    $.zxxbox.remind(msg, null, { title: "提示" });
}
